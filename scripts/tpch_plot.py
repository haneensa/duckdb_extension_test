...
tpch_opt = con.execute("""select * from tpch_df
                        where lineage_type='Logical-RID'
                          and query not in (select query from tpch_df
                                where lineage_type='Logical-OPT')
                   """).fetchdf()
...
g = ','.join(header_unique)
op_level = con.execute(f"""select t1.query, t1.sf, t1.qtype, 'Operator-Level' as lineage_type, t1.n_threads,
                    t1.nchunks, t1.lineage_size, t1.lineage_count, t1.postprocess_time,
                    t1.plan_runtime, t2.runtime, t1.output,
                    t1.mat_time+(t1.plan_runtime-t2.plan_runtime) as mat_time
                  from (select {g}, {m} from avg_tpch where lineage_type='Operator-Level') as t1
                  join (select {g}, {m} from avg_tpch where lineage_type='Operator-Level-Pass') as t2 using ({g})
                  """).df()

avg_tpch = con.execute("""select * from avg_tpch where lineage_type<>'Operator-Level' and lineage_type<>'Operator-Level-Pass'
UNION ALL select * from op_level
""").df()

# TODO: read sd_tpch_withbaseline
...
select {}, lineage_type, n_threads, output / base_output as fanout, output, nchunks, 
lineage_size, lineage_count, postprocess_time,
