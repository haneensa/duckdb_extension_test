...
mat_t*1000 as mat_overhead,
(mat_t / base_exec_t) *100 as mat_roverhead,

(all_t-base_exec_t)*1000 as all_overhead,
((all_t-base_exec_t)/base_exec_t)*100 as all_roverhead,
"""
...
if plot_filter:
    ...
    op_level = con.execute(f"""select 'Operator-Level-Test' as lineage_type, t1.op_name, t1.card, t1.sel,
                      ...
                      """).df()
    #where lineage_type<>'Operator-Level' and lineage_type<>'Operator-Level-Pass'
    filter_data_avg = con.execute("""select * from filter_data_avg
    ...
    mdata = con.execute(f"""select {g}, lineage_type, {m}, {perm_overheads}
                      from wbaseline where lineage_type='Perm' or lineage_type='Perm_list'
                      UNION ALL
                      select {g}, lineage_type, {m}, {perm_overheads}
                      from wbaseline where lineage_type='Operator-Level'
                      UNION ALL
    ...
    data = con.execute("""select 'Op-Level' as sys_label, * from data where system='Operator-Level' UNION ALL 
    SELECT 'F-Level' as sys_label, * from data where system='SmokedDuck' UNION ALL
    ...
    sample_data_card = con.execute("select * from data where overheadType<>'Total' and card=10000000").df()
...
if plot_join or plot_ineq or plot_join_mtn:
    ...
    op_level = con.execute(f"""select 'Operator-Level' as lineage_type, t1.op_name, t1.n1, t1.n2, t1.sel,
                      ...
                      """).df()
    join_data_avg = con.execute("""select * from join_data_avg where lineage_type<>'Operator-Level' and lineage_type<>'Operator-Level-Pass'
    UNION ALL select * from op_level
    """).df()
    print(op_level)
    wbaseline = con.execute(f"""select {aug_baseline}
